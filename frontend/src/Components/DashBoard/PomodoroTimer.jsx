...
export default function PomodoroTimer() {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col transition-colors duration-500 ${isDarkMode ? "bg-[#232b34] text-white" : "bg-gradient-to-br from-blue-100 to-white text-black"}`}
    >
      <Navbar />
      <div className="flex flex-col items-center justify-center flex-1 px-4 py-6">
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-8 w-full max-w-xl justify-center p-6 rounded-xl shadow-lg transition-colors duration-500">
          {[
            ...
            >
              <label className="text-sm mb-2 text-gray-800 dark:text-gray-200 font-medium">
                {input.label} (min)
              </label>
              <input
                type="number"
        ...
        <div className="flex gap-4 mb-8">
          {!isRunning ? (
            <button
              onClick={startTimer}
              className={`px-6 py-2 rounded-full font-semibold shadow-lg transform transition-transform hover:scale-105 ${isDarkMode ? "bg-blue-600 hover:bg-blue-700" : "bg-blue-400 hover:bg-blue-500 text-black"}`}
            >
              Start
            </button>
          ) : (
            <button
              onClick={pauseTimer}
              className={`px-6 py-2 rounded-full font-semibold shadow-lg transform transition-transform hover:scale-105 bg-yellow-500 hover:bg-yellow-600 text-black`}
            >
              Pause
            </button>
          )}
          <button
            onClick={resetTimer}
            className={`px-6 py-2 rounded-full font-semibold shadow-lg transform transition-transform hover:scale-105 bg-red-600 hover:bg-red-700 text-white`}
          >
            Reset
          </button>
        </div>
        ...
        <div className="flex gap-2 mb-4">
          {[...Array(SESSIONS_BEFORE_LONG_BREAK)].map((_, i) => (
            <div
              key={i}
              className={`w-5 h-5 rounded-full transition-colors duration-500 ${i < sessions % SESSIONS_BEFORE_LONG_BREAK && isWork ? "bg-green-400" : isDarkMode ? "bg-gray-600" : "bg-gray-400"}`}
            />
          ))}
        </div>
        <p
          className={`text-sm ${isDarkMode ? "text-gray-300" : "text-gray-700"} transition-colors duration-500`}
        >
