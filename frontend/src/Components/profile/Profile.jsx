...
const Profile = () => {
  ...
  const notIsEditingButton = (icon, buttonUrl, buttonName) => {
    ...
    ${buttonUrl ? "bg-gray-900 text-white hover:bg-gray-800 dark:bg-slate-700 dark:hover:bg-slate-600 cursor-pointer"
          : "bg-gray-300 text-gray-500 dark:bg-slate-800 dark:text-slate-400 cursor-not-allowed"
        }`}
      onClick={() => {
        if (buttonUrl) {
  ...
  const editingButton = (labelName, icon, name, linkName) => {
    return <>
        {/* DARK MODE: Added dark text color */}
      <label className="text-md font-semibold text-[#1D3557] dark:text-slate-200">{labelName}</label>
      <div className="flex">
        {/* DARK MODE: Adjusted background and border for the icon span */}
        <span className="bg-gray-100 px-2 py-2 rounded-l-lg border border-r-0 border-[#C5D7E5] dark:bg-slate-700 dark:border-slate-600">
          {icon}</span>
        <input
          type="text"
          name={name}
          ...
          // {/* DARK MODE: Added dark styles for the input */}
          className="w-full px-3 py-2 bg-white/70 border border-[#C5D7E5] rounded-r-lg dark:bg-slate-800/80 dark:border-slate-600 dark:text-slate-100 focus:ring-2 focus:ring-blue-500"
        />
      </div>
    </>
  }
  ...
  return (
    // {/* DARK MODE: Added dark mode gradient for the main background */}
    <div className="min-h-screen w-full bg-gradient-to-b from-[#E4ECF1] to-[#D2DEE7] dark:from-slate-900 dark:to-slate-800">
      <Navbar />
      ...
      <div className="bg-[#1d3557] text-white py-10 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0">
          {/* DARK MODE: Changed SVG fill color for dark mode */}
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" className="w-full">
            <path fill="#E4ECF1" fillOpacity="1" d="M0,160L48,149.3C96,139,192,117,288,128C384,139,480,181,576,197.3C672,213,768,203,864,181.3C960,160,1056,128,1152,117.3C1248,107,1344,117,1392,122.7L1440,128L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" className="dark:fill-slate-900"></path>
          </svg>
        </div>
      </div>
      ...
      <section className="w-full px-6 py-12 flex justify-center">
        ...
        >
          {/* DARK MODE: Added dark mode styles for the main profile card */}
          <div className="bg-gradient-to-br from-[#A4C7E6] to-[#d4e8fb] backdrop-blur-xl rounded-3xl border border-white/50 shadow-2xl overflow-hidden dark:from-slate-800 dark:to-slate-700 dark:border-slate-700">
            <div className="p-6 md:p-8">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
                {/* DARK MODE: Added dark text color */}
                <h1 className="text-3xl font-bold text-[#1D3557] flex items-center dark:text-slate-100">
                ...
                <div className="flex gap-3 flex-wrap">
                  ...
                  >
                    {isEditing ? (
                      <><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="8" y1="12" x2="16" y2="12"></line></svg>Cancel</>
                    ) : (
              ...
              <div className="grid md:grid-cols-3 gap-8">
                <div className="text-center">
                  <div className="relative mx-auto mb-6">
                    <div
                      className="relative w-40 h-40 rounded-full overflow-hidden border-4 border-white shadow-lg bg-gradient-to-br from-blue-100 to-white mx-auto dark:border-slate-500"
                    ...
                    >
                      <img
                        ...
                        onError={(e) => { e.target.src = "https://api.dicebear.com/6.x/micah/svg?seed=fallback"; }}
                      />
                      {isEditing && (
                  ...
                  {!isEditing ? (
                    <h2 className="text-2xl font-bold text-[#1D3557] dark:text-slate-100">{profileData.name}</h2>
                  ) : (
                    <input
                      type="text"
                      name="name"
                      ...
                      className="text-2xl font-bold text-center bg-white/70 border border-[#C5D7E5] rounded-lg px-2 py-1 w-full dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100"
                    />
                  )}
                  {!isEditing && (
                    <p className="text-[#1D3557]/80 dark:text-slate-400">{profileData.email}</p>
                  )}
                </div>
                ...
                <div className="md:col-span-2">
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-lg font-semibold text-[#1D3557] mb-2 dark:text-slate-200">Bio</h3>
                      {!isEditing ? (
                        <p className="text-[#1D3557]/90 dark:text-slate-300">{profileData.bio}</p>
                      ) : (
                        <textarea
                          name="bio"
                          value={editData.bio}
                          onChange={handleChange}
                          rows={3}
                          className="w-full px-4 py-3 bg-white/70 border border-[#C5D7E5] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#457B9D] dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100"
                        />
                      )}
                    </div>

                    <div>
                      <h3 className="text-lg font-semibold text-[#1D3557] mb-2 dark:text-slate-200">Details</h3>
                      <div className="grid md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm text-[#1D3557]/80 dark:text-slate-400">Location</label>
                          {!isEditing ? (
                            <p className="text-[#1D3557] dark:text-slate-300">{profileData.location || "Not specified"}</p>
                          ) : (
                            <input
                              type="text"
                              name="location"
                              ...
                              onChange={handleChange}
                              className="w-full px-3 py-2 bg-white/70 border border-[#C5D7E5] rounded-lg dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100"
                            />
                          )}
                        </div>
                        <div>
                          <label className="block text-sm text-[#1D3557]/80 dark:text-slate-400">Email</label>
                            {/* Email is not editable, so we only show the view version */}
                            <p className="w-full px-3 py-2 bg-gray-100/70 border border-[#C5D7E5] rounded-lg text-gray-500 dark:bg-slate-800/50 dark:border-slate-700 dark:text-slate-400">{editData.email}</p>
                        </div>
                        <div>
                          <label className="block text-sm text-[#1D3557]/80 dark:text-slate-400">Website</label>
                          {!isEditing ? (
                            <p className="text-[#1D3557] dark:text-slate-300">{profileData.socialLinks?.website || "Not specified"}</p>
                          ) : (
                             <input
                              type="text"
                              name="website"
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-[#1D3557] mb-2 dark:text-slate-200">Social Links</h3>
                      ...
                      <h3 className="text-lg font-semibold text-[#1D3557] mt-6 mb-2 dark:text-slate-200">Competitive Coding Profiles</h3>
                        <div className="grid sm:grid-cols-3 gap-4">
                            <div>
                                {!isEditing ? (
                    ...
                    {/* Skills Section */}
                    <div className="bg-white/60 p-6 rounded-xl shadow-md border border-blue-100 dark:bg-slate-900/40 dark:border-slate-700">
                      <h3 className="text-lg font-semibold text-[#1D3557] mb-4 flex items-center dark:text-slate-200">
                      ...
                      {isEditing ? (
                        <div className="mb-4">
                          <p className="text-sm text-gray-600 mb-2 dark:text-slate-400">Add your technical skills and technologies you're proficient in</p>
                          <div className="flex gap-2">
                            <input
                              ...
                              className="flex-grow px-3 py-2 bg-white border border-[#C5D7E5] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#457B9D] focus:border-transparent dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100 dark:placeholder-slate-400"
                        ...
                        profileData.skills.length === 0 && <p className="text-gray-500 italic mb-4 dark:text-slate-400">No skills added yet</p>
                      )}
                        ...
                        {(isEditing ? editData.skills : profileData.skills).map((skill, index) => (
                          <div
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${isEditing
                                ...
                                : "bg-[#457B9D]/10 text-[#1D3557] border border-[#457B9D]/30 hover:bg-[#457B9D]/20 dark:bg-slate-700 dark:text-slate-200 dark:border-slate-600 dark:hover:bg-slate-600"
                              } flex items-center`}
                          >
                            {isEditing ? (
                              <>
